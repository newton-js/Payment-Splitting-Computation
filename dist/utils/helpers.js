...
function getMatchedConfiguration(paymentData, feeConfigArr) {
    ...
    for (let item of feeConfigArr) {
        ...
        if ((configCurrency === paymentData.Currency || configCurrency === '*')
...
function getSpecificFeeConfig(configs) {
...
function getAppliedFee(type, value, transactionAmount) {
    let appliedFee = 0;
    if (type === 'PERC') {
